...
namespace transport_catalogue::input {
    ...
    void ReadStop(transport_catalogue::TransportCatalogue *t_cat, std::deque<std::tuple<std::string, std::string, int>>& distances);

    void ReadBusLine(std::unordered_map<std::string, std::deque<std::string>> &buses);

    void ReadInputRequests(int number_of_requests, transport_catalogue::TransportCatalogue *t_cat);
